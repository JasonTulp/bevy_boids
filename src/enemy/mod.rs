...
use rand::random;
use crate::constants::{ENEMY_COUNT, ENEMY_SPEED};
use crate::player::components::Player;
use bevy_hanabi::prelude::*;
use bevy_inspector_egui::prelude::*;
use crate::trail::spawn_trail;
use crate::window_resize::WindowState;

// pub const BOID_RADIUS: f32 = 100.0;
// pub const BOID_SEPARATION_DISTANCE: f32 = 50.0;
// pub const BOID_MAX_FORCE: f32 = 20.0;
// pub const BOID_MAX_SPEED: f32 = 500.0;
...
impl Plugin for EnemyPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, spawn_enemy_boids);
        app.add_systems(Update, boid_update);
        app.add_systems(Update, boid_flock);
        // app.add_systems(Update, wrap_around_window);
    }
}
...
/// Update the boids position and velocity
pub fn boid_update(
    mut boid_query: Query<(&mut Transform, &mut Velocity, &Acceleration, &Boid, Option<&MaxVelocity>)>,
    time: Res<Time>,
) {
...
pub fn boid_flock(
    ...
    mut boid_query: Query<(&mut Acceleration, &Transform, &Velocity, &Boid, Option<&Player>)>,
    settings: Res<Settings>,
) {
    let boid_data: Vec<(Transform, Velocity, Boid)> = boid_query.iter().map(|(_, t, v, b, _)| (t.clone(), v.clone(), b.clone())).collect();
    ...
    for (mut acceleration, transform, velocity, _, player) in boid_query.iter_mut() {
        ...
        let mut border_adjustment = Vec2::new(
            if transform.translation.x < min_x { settings.max_speed } else { 0.0 } +
                if transform.translation.x > max_x { -settings.max_speed } else { 0.0 },
            if transform.translation.y < min_y{ settings.max_speed } else { 0.0 } +
                if transform.translation.y > max_y { -settings.max_speed } else { 0.0 },
        );
        border_adjustment *= 2.0;

        *acceleration = Acceleration(
            alignment + cohesion + separation + border_adjustment
        );

    }
}

pub fn limit_vec(velocity: &mut Vec2, max_velocity: f32) {
